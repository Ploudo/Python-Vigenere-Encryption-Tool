...
Utilisation des commandes :
    ...
    Pour chiffrer le contenu d'un fichier ainsi que préciser un chemin de sortie
        ./vigenere.py -k "key" -i "InputFile.txt" -o "OutputFile.txt"
"""
...
parser = argparse.ArgumentParser(
    description="Permet de réaliser différentes commandes de chiffrement par substitution polyalphabétique (Vigenère)",
    epilog=example_text)
...
# Ici on définit la description pour les arguments utilisés :

parser.add_argument("-k", "--key", metavar="", required=False,
                    help="Définir la clef")
            # Argument pour définir une clef de chiffrement
parser.add_argument("-e", "--encrypt", metavar="", required=False,
                    help="Définir un texte à chiffrer.")
    # Argument permettant de définir le texte à chiffrer
parser.add_argument("-EF", "--encryptFile", metavar="", required=False,
                    help="Définir un fichier à chiffrer.")
    # Argument permettant de chiffrer le texte d'un fichier
parser.add_argument("-d", "--decrypt", metavar="", required=False,
                    help="Définir un texte à déchiffrer.")
    # Argument permettant à déchiffrer un texte
parser.add_argument("-DF", "--decryptFile", metavar="", required=False,
                    help="Définir un fichier à déchiffrer.")
    # Argument permettant à déchiffrer le texte d'un fichier
parser.add_argument("-OF", "--outputFile", metavar="", required=False,
                    help="Définir un fichier de sortie.")
    ...
    #ArgParse traite toutes les options qu'on lui donne comme des strings. Si on aurait voulu utiliser des
    #chiffres ou des nombres, on doit lui préciser avec un 'type=int' qui suit le 'help'.

args = parser.parse_args()
    # Alias pour la commande parser.parse_args()
...
def code(key):
    if key.islower():  # Si la lettre est une minuscule.
...
if __name__ == '__main__':
    ...
    if args.key and not (args.encrypt or args.encryptFile or args.decrypt or args.decryptFile):
    ...
    elif args.key and args.encrypt:  # Si l'utilisateur choisit -k et -e (Key et Encrypt)
        ...
        if args.outputFile:  # Si l'utilisateur choisit l'argument -OF (Output File)
            vigenere_output = open(args.outputFile, mode="w")  # Créer un fichier avec open() en mode écriture.
    ...
    elif args.key and args.encryptFile:  # Si l'utilisateur choisit -k et -EF (Key et Encrypt File)
    ...
    elif args.key and args.decrypt:  # Si l'utilisateur choisi -k et -d (Key et Decrypt)
        ...
        if args.outputFile:  # Si l'utilisateur a choisi l'argument -OF (Output File)
            vigenere_output = open(args.outputFile, mode="w")  # Créer un fichier avec open() en mode écriture.
            vigenere_output.write(encrypt(args.key, args.decrypt))  # Écrire le résultat dans le fichier
            print(f"Encrypted to: {args.outputFile}")  # Avertir l'utilisateur du bon déroulement de l'opération
            vigenere_output.close()  # Fermer le fichier

    elif args.key and args.decryptFile:  # Si l'utilisateur choisi -k et -DF (Key et Decrypt File)
        ...
        if args.outputFile:  # Si l'utilisateur choisit l'argument -OF (Output File)
            ...
            print(f"Decrypted to: {args.outputFile}")  # Avertir l'utilisateur que l'opération s'est terminé avec succès.
            vigenere_output.close()  # Fermer le fichier
        ...
        print(
            "Mauvaise utilisation de la commande ! Taper 'python vigenere.py -h' pour la liste des commandes.\n")
        # En cas d'erreur de frappe / commande inconnue etc.... (tout ce qui n'aurait pas fonctionné)
        parser.print_help()

# Author : danglock 'ЯΛłКФ#1106', AlexGirardin 'AlexioShow#1167', Ploudo 'Ploudo#3585'
